...
// the tokenVersion claim (the session-revocation counter). There is no
// workspace claim anymore: workspace pages address the workspace by URL slug,
// and the workspace guard resolves + re-validates it per request. The DB row
// is the source of truth: the auth guard re-reads tokenVersion per request and
// rejects any token signed before a bump.
...
export function signAuthToken({
	...
	return new SignJWT({ accountId, tokenVersion })
