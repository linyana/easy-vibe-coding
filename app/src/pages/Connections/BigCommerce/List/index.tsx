...
export function BigCommerceConnectionList({
	onAction,
}: {
	onAction: (action: ConnectionsAction) => void;
}) {
	const list = useAPIList({
		// This page's rows are exactly the bigcommerce rows — the platform
		// filter rides the query so pagination/search never cross platforms.
		queryKey: ['connections', 'bigcommerce'],
		initialSearch: { platform: 'bigcommerce' },
	...
	usePageHeader({ title: 'BigCommerce' });

	return (
		<div className="space-y-4">
			<Card
				icon={{ name: 'Plug' }}
				title="BigCommerce connections"
				description="BigCommerce accounts — store the credentials the product API calls out with."
				actions={
					<Button onClick={() => onAction({ kind: 'create' })}>
						<PlusIcon className="size-4" />
						Add BigCommerce connection
					</Button>
				}
			>
				<div className="space-y-4">
					...
					<ListTable
						list={list}
						columns={columns}
						emptyMessage="No BigCommerce connections yet"
					/>
				</div>
			</Card>
		</div>
