...
- **身份**：`slug` 是 workspace 的用户可见唯一标识（unique 必填，URL-safe：小写字母/数字/单连字符）；int 主键只在 DB 内部（FK 完整性、改名安全），任何 API 表面、token claim、前端都以 slug 寻址。**唯一例外：workspace 上下文 ref（`WorkspaceRef`，session 面）携带 `id`**——admin 的 workspace 面（member/permission）按 id 寻址成员数据（admin members 端点本来就是 by-id），ref 带 id 让页面从上下文直接取到地址，不靠 URL 参数。slug 冲突 → 409。
