...
7. **列表过滤状态在组件内存里**（`useState`，经 `useAPIList`），不进 URL。URL 保持干净；不要给列表路由加 `validateSearch`。（唯一例外：workspace 页的 `:slug` 路径段是**地址**不是过滤状态——多租户的 URL 寻址本身就是系统语义。）
...
- **地址模型**：workspace 由 **URL slug 寻址**，token 里没有任何 workspace。页面路径带 slug（用户面 `/workspaces/:slug/*`、admin 进入面 `/admin/workspaces/:slug/*`）；API 每请求带 `X-Workspace-Slug` header（`libs/api` 的 headers 钩子读 slug 路由 beforeLoad 写入的 request-scope 状态），`workspace`/`role` guard 每请求按 header 解析 slug → 查行 → 校验。**URL 是唯一事实源**：切 workspace = 纯导航（无 token 交换、无 `/me` 回显）；改 slug = 旧链接失效（壳的 403/404 引导回 picker），没有"在途会话"要保。slug 冲突 → 409。
- **身份**：`slug` 是用户可见唯一标识（unique、URL-safe：小写字母/数字/单连字符）；int 主键内部用（FK、管理端点按 `:id`）。展示/缓存用的 ref（`WorkspaceRef { id, slug, name }`）由 slug 壳的 `WorkspaceProvider` 详情 fetch 后写入 `useGlobal.workspace`（内存-only，卸载即清，只供展示 + queryKey，不寻址）；`localStorage` 的 last-workspace slug 只决定 `/` 落在哪，不是事实源。
...
- **用户表面**：`/personal/workspaces` 是 picker（`GET /workspaces` 成员列表 + 创建），选择 = 导航进 `/workspaces/:slug`；workspace 页挂在 `/workspaces/:slug/members`、`/workspaces/:slug/connections/…`（`role: ['owner', 'member']` + disabled 门）。壳分两层：`routes/workspaces.tsx`（session gate）+ `routes/workspaces/$slug.tsx`（`WorkspaceProvider`：fetch `GET /workspaces/:slug`——成员被移除/disabled/不存在 → 403/403/403 → 壳内错误态 + 回 picker；`beforeLoad` 写 request-scope slug）。`/` 按 last slug 落，没有则回 picker。
- **admin 表面**：平台面在 `modules/admin/workspaces`（prefix `/workspaces/admin`，`admin: true`）：列表/统计/编辑/删除/开关按 `:id`；进入面的壳元数据是 `GET /workspaces/admin/current`（`admin: true` + `workspace: true`，workspace 从 header slug 解析——无成员门、disabled 也可进；**静态路径**：不挂 `/:slug`，避免与 `:id` 在 Eden 上撞动态参数）。成员管理独立成 `modules/admin/members`（prefix `/admin/workspaces/members`，`admin: true` + `workspace: true`：workspace 从 header slug 解析——admin 非成员也能管理，URL 无 workspace id：`GET` · `POST`（按邮箱加人，账号不存在 404、重复 409）· `PATCH /:accountId`（改 role）· `DELETE /:accountId`）。**不变式**：workspace 必须保持 ≥1 个 owner——降级/移除最后一个 owner → 409（`assertNotLastOwner`）。删除 workspace 级联清成员（FK cascade）。
...
- **守卫**：`api/src/libs/guards` 按层拆分三个模块：`auth/`（`auth` + `admin` macro）、`workspace/`（`workspace` macro）、`role/`（`role` macro）——`workspace` 建立在 `auth` 上、`role` 建立在 `workspace` 上（`.use()` 组合）；`index.ts` 导出组合实例 `guards`（controllers 一律 `.use(guards)`）。`admin: true` 在验 token 之外**每次请求重查 DB 行的 isAdmin**（行是事实源，撤销立即生效，不等 token TTL）。**fail-closed 规则**：workspace macro 按 header slug 解析后**不带授权**——任何登录账号都能提交任意 slug，所以它只允许两种组合：用户数据端点必须带 `role`（membership 校验），admin 的 workspace 面用 `admin: true` + `workspace: true`；**禁止裸 `workspace: true` 挂用户数据**。
- **边界**：admin 不能删除自己、不能撤销自己的 isAdmin（400）——保证平台永远至少有一个 admin。批量删除含自己 → 整批 400（显式拒绝，不是静默跳过）。
- **表面**：`accounts` 模块在 `modules/admin/accounts`，整块 admin-only（平台级账号 CRUD + `PATCH /:id/password` 重置密码——密码字段永不进通用 PATCH）；`workspaces` 的 admin 端点在 `modules/admin/workspaces`（见 Workspaces 节），admin 的 entered-workspace 成员管理在 `modules/admin/members`。**模块归属规则：作用域决定位置——用户级模块在 `modules/<name>`，平台级 admin 模块在 `modules/admin/<name>`（镜像 app 侧 `pages/` + `pages/Admin/`）。** UI 是 `/admin` 壳（非 pathless，不依赖 workspace 上下文；`beforeLoad` token 守卫 + `useSession` + `isAdmin` 门），**复用普通用户布局**（`LayoutProvider` + 专属 `AdminSidebar` + `SiteHeader`/`usePageHeader`）。**admin 无应用内入口/切换器**：admin 面只靠 URL 直达（`/admin`，isAdmin 门守卫），`/` 对 admin 与普通用户无异（workspace picker 不含 admin 卡片）。admin 是平台级、与 workspace 上下文正交。

**admin 进入 workspace**：`/admin/workspaces` 平台列表**整行点击即进入**（Actions 也有 Enter 项）= 导航到 `/admin/workspaces/:slug/member`（纯 URL——无 token 交换、无成员校验：admin 可进入任意 workspace，`admin: true` 就是门）。壳是 `routes/admin/workspaces/$slug.tsx`（`WorkspaceProvider` variant=admin：fetch `GET /workspaces/admin/current`，disabled workspace 也能进），Member（成员管理，复用 admin members 端点）与以后的 Permission 作为同级子路由；`AdminWorkspaceSwitcher` 就是导航到另一 slug（整页重作用域，因为 URL 变了）。**AdminSidebar 按路径切换两种模式**：GLOBAL（`/admin/*` 除进入面，即 `/admin/workspaces` 列表也算）= Banner + 静态上下文行 + Overview/Accounts/Workspaces；WORKSPACE（`/admin/workspaces/:slug/*`）= **Back to admin**（→ `/admin/workspaces`）退出 + workspace 卡片（可点开 switcher）+ Member / Permission 占位导航。回普通应用只靠 URL `/`（无导航项）。

## Auth

- **机制**：JWT bearer（jose，HS256，7 天），密码 argon2id（`Bun.password`，零依赖）。**token 只带账号身份**：`accountId` claim（`sub` 镜像）+ `tokenVersion` claim（会话撤销计数器，`libs/auth` 签发时带当前行值，`auth` guard 每次请求比对）——**没有任何 workspace 内容**：workspace 是 URL/请求级概念。token 存 `useGlobal`（persist），`libs/api` 的 headers 钩子逐请求注入；同一钩子把当前 slug 路由写入的 request-scope slug 带成 `X-Workspace-Slug`。`onResponse` 全局拦截 401 → 清 session。
- **服务端守卫**（`api/src/libs/guards`）：纯 jose 原语（`libs/auth`）之上的 Elysia 适配层，三个分层模块组合（`guards/auth` → `guards/workspace` → `guards/role`，`index.ts` 导出组合实例 `guards`）——`admin`/`workspace`/`role` 都声明 `auth: true` 组合复用验 token 逻辑，不复写。`auth: true` 验 bearer token（不过 401），注入 `{ auth: { accountId } }`，且**重查账号行**：账号被删 → 401（删除立即生效，不等 TTL），`tokenVersion` claim ≠ 行内计数器 → 401（会话已撤销）。`admin: true` 是平台 admin 门：**每次请求重查 DB 行的 isAdmin**，非 admin → 403——**admin 不是 role，绝不混进角色列表**。`workspace: true` 解析请求的 `X-Workspace-Slug` header → 查行（不存在 → 403）并**注入 `workspace { id, slug, name, disabled }` 到 context**，handler 从 `({ workspace })` 读；它**不带授权**（fail-closed 见 Admin 节）。`role: [...]` 是函数式 macro，option 值即允许的 **workspace 成员角色列表**（`'owner' | 'member'`，词汇来自 shared 的 `memberRoleSchema`），声明 `workspace: true` 依赖作安全网（保证 workspace guard 先跑，显式声明时去重只执行一次），role 只判成员身份（按 `workspace.id` + accountId 查 membership 行），**不自己解析 slug**；**精确匹配：列表里有什么角色才放行什么——owner 不满足 `['member']`，"任意成员"写成 `['owner', 'member']`**；disabled 门在 role（被关闭的 workspace 对成员表现为已删除，admin 的 admin+workspace 面不受阻）。**重置密码必须同时 bump `tokenVersion`**，否则旧会话在 TTL 内继续有效。**新模块加一行 `.use(guards)` 即接入；workspace 作用域的 feature 显式写 `.guard({ workspace: true, role: ['owner', 'member'] })`（任意成员）或 `['owner']` 等精确列表（role 的内部依赖作安全网），handler 收 `workspace.id`——不自己判 slug、不自己 join slug→id。admin 平台面用 `.guard({ admin: true })`（见 Admin 节）；admin 的 workspace 面（成员管理）用 `admin: true` + `workspace: true`。**
- **API**：`/api/auth/register`（建号即登录；**第一个注册的账号自动成为 admin**）· `/api/auth/login`（大小写不敏感匹配邮箱，错误统一 401 "Invalid email or password" 不泄露邮箱）· `/api/auth/me`（guard 解析 accountId → 服务重查当前账号行，回 `{ account }`——**不回显 workspace**：workspace 由各 slug 页的详情端点按需解析）。`switch-workspace` / admin switch 端点已删除（URL 寻址替代）。`accounts` 模块整块在 `admin: true` 守卫后。契约在 `packages/shared/src/api/auth/`，密码字段规则复用 accounts 的 `accountFieldSchemas`。
- **路由**：workspace 应用面 = `routes/workspaces.tsx`（`beforeLoad` session gate + `SessionProvider` 的 me 校验，loading/error/unauthenticated 三分支）+ `routes/workspaces/$slug.tsx`（WorkspaceProvider + LayoutProvider）；个人壳 `routes/personal.tsx`（picker / 账号 / LLM）；平台壳 `routes/admin.tsx`（+ isAdmin 门）。`/login` `/register` 在壳外，读 `redirect` search 参数（登录后回跳，`safeRedirect` 防开放重定向）；`/` 是 hub：有 last slug 就落 `/workspaces/:slug`，否则落 picker。
