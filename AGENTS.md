...
- **用户表面**：`/profile/workspaces` 是 picker（`GET /workspaces` 成员列表 + 创建），选择 = 导航进 `/workspaces/:slug`；workspace 页挂在 `/workspaces/:slug/members`、`/workspaces/:slug/connections/…`（`role: ['owner', 'member']` + disabled 门）。壳分两层：`routes/workspaces.tsx`（session gate）+ `routes/workspaces/$slug.tsx`（`WorkspaceProvider`：fetch `GET /workspaces/:slug`——成员被移除/disabled/不存在 → 403/403/403 → 壳内错误态 + 回 picker；`beforeLoad` 写 request-scope slug）。`/` 按 last slug 落，没有则回 picker。
...
- **路由**：workspace 应用面 = `routes/workspaces.tsx`（`beforeLoad` session gate + `SessionProvider` 的 me 校验，loading/error/unauthenticated 三分支）+ `routes/workspaces/$slug.tsx`（WorkspaceProvider + LayoutProvider）；个人壳 `routes/profile.tsx`（picker / 账号 / LLM）；平台壳 `routes/admin.tsx`（+ isAdmin 门）。`/login` `/register` 在壳外，读 `redirect` search 参数（登录后回跳，`safeRedirect` 防开放重定向）；`/` 是 hub：有 last slug 就落 `/workspaces/:slug`，否则落 picker。
